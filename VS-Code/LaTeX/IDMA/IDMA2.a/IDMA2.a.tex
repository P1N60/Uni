...
For bubblesort er en arrat, som er sorteret i decreasing order den worst case, da for-loop'et kører gennem alle elementer i array'en, og while-loop'et kører $n$ gange. Vi har derfor den samme worst case asympototic time complexity som jeg redegjorde for tidligere, nemlig $O(n^2)$.\\
\\
merge sort: $O(n \cdot log(n))$\\
